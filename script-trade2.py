...
LOT_SIZE = 0.2    # Ukuran lot per entry
        ...
        if "Your XAUUSD alert was triggered" in email_body:
            ...
            if match:
                ...
                if "New strategy position is 0" in email_body:
                    print(f"🚨 EXIT SIGNAL DETECTED: Closing all positions at price {price}")
                    close_all_positions()
                else:
                    print(f"🚨 ENTRY SIGNAL DETECTED: Opening two {direction} positions at price {price}")
...
def execute_entry(direction, price):
    try:
        # Kirim dua order entry ke MT5
        for _ in range(2):  # Entry dua kali
            request = {
...
def close_all_positions():
    try:
        # Dapatkan semua posisi terbuka untuk simbol tertentu
        ...
        for position in positions:
            ...
            if result.retcode != mt5.TRADE_RETCODE_DONE:
                ...
                print(f"Position {position.ticket} closed successfully: {result}")
    except Exception as e:
